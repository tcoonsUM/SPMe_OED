...
def run_battery_simulation(a_nmc, b_nmc, c_nmc, d_nmc, my_graphite_diff_parameter, sep_por, neg_por, pos_por, cap_dl_neg, p1, p2, p3, p4, p5, p6, p7, p8):
    ...
    parameters_1['Initial concentration in negative electrode [mol.m-3]'] = 2392.8 # had +11000
    parameters_1['Initial concentration in positive electrode [mol.m-3]'] = 28588 #had -11000
    ...
    def create_custom_experiment(p1,p2,p3,p4,p5,p6,p7,p8):
        ...
        # p5 = 100
        # p9 = 200
        extended_duration_factor = 4
        
        t = np.linspace(0, p8*extended_duration_factor, int(2501*extended_duration_factor)) #change this time to be one period of the smallest sine (or a quarter of a period)  #start stop num #see Nyquist theorem - 2501 comfortably fits a 1 Hz limit 
        ...
        drive_cycle_power = np.column_stack([t, chirp_signal(p7,p8*extended_duration_factor)])
        p9 = 200 #default
        p10 = 300 #default
        experiment = pybamm.Experiment( [pybamm.step.current(drive_cycle_power),
            (f"Rest for {p9} seconds", f"Charge at {p1}C for {p2} seconds or until 4.2 V", f"Rest for {p3} seconds", f"Charge at {p4}C for {p5} seconds or until 4.2 V", f"Rest for {p6} seconds", f"Discharge at {p4}C for {p5} seconds or until 3.0 V", f"Rest for {p10} seconds")
        ] * 5 )
        return experiment
...
#     model_SPMe_1.latexify("spm_equations_.png")

#     # Print LHS and RHS for differential equations
#     print("ODEs (LHS = RHS):")
#     for lhs_variable, rhs_equation in model_SPMe_1.rhs.items():
#         print(f"d({lhs_variable})/dt = {rhs_equation}")

#     # Print algebraic equations (implicitly zero on LHS)
#     print("\nAlgebraic Equations (Implicit LHS - Zero = RHS):")
#     for lhs_variable, rhs_equation in model_SPMe_1.algebraic.items():
#         print(f"0 = {rhs_equation}")
    #return sim_1.solution
    time = sim_1.solution["Time [s]"].data
    current = sim_1.solution["Current [A]"].data

    terminal_voltage = sim_1.solution['Terminal voltage [V]'].data
    return time, current, terminal_voltage