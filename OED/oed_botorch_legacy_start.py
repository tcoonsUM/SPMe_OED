...
def bo_friendly_objective(d0, d1, d2, d3, d4, d5, d6, d7, n_in, n_out, rel_std, clusters_inds_npz, corrs_clustered_npz):
    ...
    dur = 5*(500 + d1prime + d2 + 2*d4 + d5 + 4*d7prime)
    utilities = ute.eig_mp(d, n_in, n_out, rel_std, clusters_inds_npz, corrs_clustered_npz)
    return utilities.mean()/dur

def compute_dur(X, bounds):
    # takes unscaled (GP coordinates) X
    d = inverse_min_max_scale(X, bounds[0], bounds[1])
    d1prime = d[:,1]/d[:,0]
    d7prime = d[:,7]/d[:,6]
    durs = 5*(500 + d1prime + d[:,2] + 2*d[:,4] + d[:,5] + 4*d7prime)
    return durs.unsqueeze(1)

def compute_dur_unscaled(X):
    # takes X in real units
    d = X
    durs = 5*(500 + d[:,1] + d[:,2] + 2*d[:,4] + d[:,5] + 4*d[:,7])
    return durs.unsqueeze(1)
...
if __name__ == "__main__":
    ...
    train_Y_unscaled_legacy = torch.load("train_Y_unscaled_6e3.pt")
    ...
    train_X_scaled = min_max_scale(train_X_new_coords, bounds[0], bounds[1]).to(torch.float64)
    durs = compute_dur(train_X_scaled, bounds)
    train_Y = train_Y_unscaled_legacy/durs  # Shape [n_points, 1]
    ...
    for iteration in range(n_iterations):
        ...
        torch.save(train_X_unscaled_tensor, "train_X_unscaled_cont.pt")
        torch.save(train_Y_unscaled_tensor, "train_Y_unscaled_cont.pt")
